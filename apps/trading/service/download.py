...
def patch_realtime_data(df, match_price, chart_type):
    ...
    try:
        ...
        if chart_type_val == CandleEnum.W1.value:
            # ── W1: KHÔNG patch realtime ──
            # download_data_weekly đã tự aggregate D1 data (bao gồm hôm nay nếu thị trường mở),
            # hình thành nến tuần với close = close D1 cuối cùng của tuần → khớp TradingView.
            # Nếu patch thêm matchPrice vào close của nến tuần → stoch_rsi/macd tính sai,
            # gây lệch D0/D1/D2 so với thực tế trên sàn.
            return
